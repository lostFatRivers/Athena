...
public class RedisClient {
	...
	public boolean lock(String key, String requireId) {
		try (Jedis jedis = jedisPool.getResource()) {
			...
			return Constants.LOCK_SUCCESS.equals(result);
		} catch (Exception e) {
	...
	public boolean unlock(String key, String requireId) {
		try (Jedis jedis = jedisPool.getResource()) {
			Object result = jedis.eval(Constants.UNLOCK_SCRIPT, Collections.singletonList(key),
					Collections.singletonList(requireId));
			if (!Constants.UNLOCK_SUCCESS.equals(result)) {
